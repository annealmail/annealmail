...
test(function extractMessageIdExtractsARegularMessageId() {
  var result = AnnealMailData.extractMessageId("annealmail:message/foobar");
  Assert.equal("foobar", result);
});

test(function extractMessageIdReturnsAnEmptyStringWhenItCantMatch() {
  var result = AnnealMailData.extractMessageId("annealmail:mime-message/foobar");
  Assert.equal("", result);
});

// testing: extractMimeMessageId
test(function extractMimeMessageIdExtractsARegularMessageId() {
  var result = AnnealMailData.extractMimeMessageId("annealmail:mime-message/fluff");
  Assert.equal("fluff", result);
});

test(function extractMimeMessageIdReturnsAnEmptyStringWhenItCantMatch() {
  var result = AnnealMailData.extractMimeMessageId("annealmail:message/mess");
  Assert.equal("", result);
});
