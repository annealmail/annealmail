/*global Components: false, EnigInitCommon: false, AnnealMailDialog: false */
/*
...
/* global EnigGetPref: false, EnigGetString: false, EnigFormatFpr: false, EnigGetTrustLabel: false, nsIAnnealMail: false */
/* global GetAnnealMailSvc: false, EnigConfirm: false, EnigAlert: false, EnigShowPhoto: false, EnigFilePicker: false */
...
// imported packages
/* global AnnealMailLog: false, AnnealMailEvents: false, AnnealMailKeyRing: false, AnnealMailWindows: false, AnnealMailKeyEditor: false */
/* global AnnealMailKey: false, AnnealMailLocale: false, AnnealMailPrefs: false */

// Initialize annealmailCommon
EnigInitCommon("annealmailKeyManager");
...
function annealmailKeyManagerLoad() {
  AnnealMailLog.DEBUG("annealmailKeyManager.js: annealmailKeyManagerLoad\n");
  ...
  gUserList.addEventListener('click', annealmailOnClick, true);
  ...
  AnnealMailEvents.dispatchEvent(loadkeyList, 100, null);
...
function loadkeyList() {
  AnnealMailLog.DEBUG("annealmailKeyManager.js: loadkeyList\n");
...
function refreshKeys() {
  AnnealMailLog.DEBUG("annealmailKeyManager.js: refreshKeys\n");
...
function buildKeyList(refresh) {
  AnnealMailLog.DEBUG("annealmailKeyManager.js: buildKeyList\n");
  ...
  keyListObj = AnnealMailKeyRing.getAllKeys(window, getSortColumn(), getSortDirection());
...
function annealmailOnClick(event) {
  if (event.detail != 2) {
    return;
  }
  ...
  if (keyType == "uat") {
    ...
    annealmailKeyDetails();
  }
}

function annealmailSelectAllKeys() {
  gUserList.view.selection.selectAll();
}

function annealmailKeyDetails() {
  var keyList = getSelectedKeys();
  if (keyList.length > 0) {
    if (AnnealMailWindows.openKeyDetails(window, gKeyList[keyList[0]].keyId, false)) {
      refreshKeys();
    }
  }
}
...
function annealmailDeleteKey() {
  ...
  AnnealMailKeyEditor.deleteKey(window, fprArr.join(" "), deleteSecret,
...
function annealmailEnableKey() {
  ...
  function processNextKey() {
    AnnealMailKeyEditor.enableDisableKey(window, "0x" + gKeyList[keyList[keyIndex]].keyId, disableKey, function _enDisCb(exitCode, errorMsg) {
      if (exitCode === 0) {
        ++keyIndex;
...
function keyMgrAddPhoto(userId, keyId) {
  ...
  window.openDialog("chrome://annealmail/content/annealmailImportPhoto.xul", inFile, "chrome,modal=1,resizable=1,dialog=1,centerscreen", argsObj);

  if (!argsObj.okPressed) return;

  AnnealMailKeyEditor.addPhoto(window, "0x" + keyId, inFile,
    function(exitCode, errorMsg) {
      if (exitCode !== 0) {
...
function enigCreateKeyMsg() {
  ...
  var exitCodeObj = {};
  var errorMsgObj = {};
  AnnealMailKeyRing.extractKey(false, "0x" + keyList.join(" 0x"), tmpFile, exitCodeObj, errorMsgObj);
...
function annealmailExportKeys() {
  ...
  var exitCodeObj = {};
  var errorMsgObj = {};
  AnnealMailKeyRing.extractKey(exportSecretKey, keyListStr, outFile, exitCodeObj, errorMsgObj);
  if (exitCodeObj.value !== 0) {
...
function annealmailImportKeysFromFile() {
  ...
  var errorMsgObj = {};
  // preview
  var preview = AnnealMailKey.getKeyListFromKeyFile(inFile, errorMsgObj);
  ...
  if (preview.length > 0) {
    ...
    if (exitStatus) {
      // import
      var keyListObj = {};
      var exitCode = AnnealMailKeyRing.importKeyFromFile(inFile, errorMsgObj, keyListObj);
      if (exitCode !== 0) {
    ...
    if (AnnealMailKeyRing.getCacheEmpty()) {
      refreshKeys();
    }
  }
}
...
function enigGetClipboard() {
  AnnealMailLog.DEBUG("annealmailKeyManager.js: enigGetClipboard:\n");
  var cBoardContent = "";
  ...
  try {
    ...
    AnnealMailLog.DEBUG("annealmailKeyManager.js: enigGetClipboard: got data\n");
  }
  catch (ex) {}
  return cBoardContent;
}

function annealmailImportFromClipbrd() {
  var annealmailSvc = GetAnnealMailSvc();
  if (!annealmailSvc)
    return;
  ...
  var errorMsgObj = {};
  var preview = AnnealMailKey.getKeyListFromKeyBlock(cBoardContent, errorMsgObj);
  var exitStatus = -1;
  ...
  if (preview.length > 0) {
    ...
    if (exitStatus) {
      // import
      var r = AnnealMailKeyRing.importKey(window, false, cBoardContent, "", errorMsgObj);
      var keyList = preview.map(function(a) {
        return a.id;
      });
      AnnealMailDialog.keyImportDlg(window, keyList);
...
function annealmailCopyToClipbrd() {
  var annealmailSvc = GetAnnealMailSvc();
  if (!annealmailSvc)
    return;
  ...
  var exitCodeObj = {};
  var errorMsgObj = {};
  var keyData = AnnealMailKeyRing.extractKey(0, "0x" + keyList.join(" 0x"), null, exitCodeObj, errorMsgObj);
  if (exitCodeObj.value !== 0) {
  ...
  try {
    ...
    AnnealMailLog.DEBUG("annealmailKeyManager.js: annealmailImportFromClipbrd: got data from clipboard");
...
function annealmailUploadKeys() {
  annealmailKeyServerAccess(nsIAnnealMail.UPLOAD_KEY, annealmailUploadKeysCb);
}

function annealmailUploadKeysCb(exitCode, errorMsg, msgBox) {
  if (msgBox) {
    if (exitCode !== 0) {
...
function annealmailReceiveKey() {
  annealmailKeyServerAccess(nsIAnnealMail.DOWNLOAD_KEY, annealmailReceiveKeyCb);
}

function annealmailRefreshAllKeys() {
  var checkedObj = {};
  var doIt = false;
  ...
  if (doIt) annealmailKeyServerAccess(nsIAnnealMail.REFRESH_KEY, annealmailReceiveKeyCb);
}

// Iterate through contact emails and download them
function annealmailDowloadContactKeysEngine() {
  ...
  while (allAddressBooks.hasMoreElements()) {
    ...
    if (addressBook instanceof Ci.nsIAbDirectory) { // or nsIAbItem or nsIAbCollection
      ...
      var doIt = AnnealMailDialog.confirmDlg(window,
...
function annealmailDownloadContactKeys() {

  var doIt = AnnealMailDialog.confirmPref(window,
  ...
  if (doIt) annealmailDowloadContactKeysEngine();
}
...
function annealmailReceiveKeyCb(exitCode, errorMsg, msgBox) {
  AnnealMailLog.DEBUG("annealmailKeyManager.js: annealmailReceiveKeyCb\n");
  if (msgBox) {
    if (exitCode === 0) {
      refreshKeys();
      AnnealMailEvents.dispatchEvent(displayResult, 100, [EnigGetString("receiveKeysOk"), errorMsg]);
    }
    else {
      AnnealMailEvents.dispatchEvent(displayResult, 100, [EnigGetString("receiveKeysFailed"), errorMsg]);
    }
  }
  else {
...
function addToPRRule() {
  ...
  window.openDialog("chrome://annealmail/content/annealmailSelectRule.xul",
    "", "dialog,modal,centerscreen", inputObj);

}

function annealmailImportKeysFromUrl() {
  var value = {
    "value": ""
  };
  if (AnnealMailDialog.promptValue(window, EnigGetString("importFromUrl"), value)) {
    var p = new Promise(
      function(resolve, reject) {
        var cbFunc = function _cb(data) {
          AnnealMailLog.DEBUG("annealmailImportKeysFromUrl: _cbFunc()");
          var errorMsgObj = {};

          // preview
          var preview = AnnealMailKey.getKeyListFromKeyBlock(data, errorMsgObj);
          var exitStatus = -1;
          ...
          if (preview.length > 0) {
            ...
            if (exitStatus) {
              AnnealMailKeyRing.importKey(window, false, data, "", errorMsgObj);
        ...
        try {
          var bufferListener = AnnealMailStreams.newStringStreamListener(cbFunc);
...
function annealmailToggleShowAll() {
...
//
function annealmailKeyServerAccess(accessType, callbackFunc) {
  ...
  if (accessType != nsIAnnealMail.REFRESH_KEY) {
  ...
  let autoKeyServer = AnnealMailPrefs.getPref("autoKeyServerSelection") ? AnnealMailPrefs.getPref("keyserver").split(/[ ,;]/g)[0] : null;
  if (autoKeyServer) {
    resultObj.value = autoKeyServer;
  }
  else {
    window.openDialog("chrome://annealmail/content/annealmailKeyserverDlg.xul",
  ...
  window.openDialog("chrome://annealmail/content/enigRetrieveProgress.xul",
    "", "dialog,modal,centerscreen", keyDlObj, resultObj);

  if (accessType != nsIAnnealMail.UPLOAD_KEY && resultObj.result) {
    refreshKeys();
  }
}
    ...
    switch (r.rowType) {
      ...
      case "uat":
        switch (keyObj.keyTrust) {
          case 'q':
            keyTrustStyle = "annealmail_keyValid_unknown";
            break;
          case 'i':
            keyTrustStyle = "annealmail_keyValid_invalid";
            break;
          case 'd':
            keyTrustStyle = "annealmail_keyValid_disabled";
            break;
          case 'r':
            keyTrustStyle = "annealmail_keyValid_revoked";
            break;
          case 'e':
            keyTrustStyle = "annealmail_keyValid_expired";
            break;
          case 'n':
            keyTrustStyle = "annealmail_keyTrust_untrusted";
            break;
          case 'm':
            keyTrustStyle = "annealmail_keyTrust_marginal";
            break;
          case 'f':
            keyTrustStyle = "annealmail_keyTrust_full";
            break;
          case 'u':
            keyTrustStyle = "annealmail_keyTrust_ultimate";
            break;
          case '-':
            keyTrustStyle = "annealmail_keyTrust_unknown";
            break;
          default:
            keyTrustStyle = "annealmail_keyTrust_unknown";
            break;
        ...
        if (r.rowType === "key" && keyObj.secretAvailable) {
          keyTrustStyle += " annealmailOwnKey";
        }
        break;
    }
    ...
    switch (r.rowType) {
        ...
        switch (col.id) {
          ...
          case "typeCol":
            if (keyObj.secretAvailable) {
              return AnnealMailLocale.getString("keyType.publicAndSec");
            }
            return AnnealMailLocale.getString("keyType.public");
          case "validityCol":
            if (keyObj.keyUseFor.indexOf("D") >= 0) {
              return AnnealMailLocale.getString("keyValid.disabled");
            }
      ...
      case "uidHdr":
        if (col.id === "enigUserNameCol") {
          return AnnealMailLocale.getString("keylist.hasOtherUids");
        }
        break;
      case "noUidHdr":
        if (col.id === "enigUserNameCol") {
          return AnnealMailLocale.getString("keylist.noOtherUids");
        }
        break;
      case "uat":
        if (col.id === "enigUserNameCol") {
          return AnnealMailLocale.getString("userAtt.photo");
        }
        break;
      case "uatHdr":
        if (col.id === "enigUserNameCol") {
          return AnnealMailLocale.getString("keylist.hasPhotos");
        }
        break;
      case "noUatHdr":
        if (col.id === "enigUserNameCol") {
          return AnnealMailLocale.getString("keylist.noPhotos");
        }
        break;
    }
