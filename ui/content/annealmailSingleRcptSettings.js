...
/* global EnigInitCommon: false, GetAnnealMailSvc: false, EnigAlert: false, EnigGetString: false */
/* global EnigConfirm: false, AnnealMailLog: false, AnnealMailKeyRing: false */

"use strict";

// Initialize annealmailCommon
EnigInitCommon("annealmailSingleRcptSettings");
...
function annealmailDlgKeySelection() {
  AnnealMailLog.DEBUG("annealmailMsgComposeHelper.js: annealmailDlgKeySelection: \n");
  ...
  if ((keyList.length === 0) || (keyList.length == 1 && keyList[0].length === 0)) {
    ...
    for (let i = 0; i < keyList.length; i++) {
      ...
      if (keyList[i].indexOf("GROUP:") === 0) {
        ...
        let keyObj = AnnealMailKeyRing.getKeyById(keyList[i]);
        if (keyObj) {
