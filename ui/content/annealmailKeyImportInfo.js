...
function onLoad() {
  var dlg = document.getElementById("annealmailKeyImportInfo");
  ...
  dlg.setAttribute("title", AnnealMailLocale.getString("importInfoTitle"));
  ...
  let onClickFunc = function(event) {
    ...
    AnnealMailWindows.openKeyDetails(window, keyId, false);
  };
  ...
  for (i = 0, keys = []; i < keyList.length; i++) {
    ...
    let keyObj = AnnealMailKeyRing.getKeyById(keyId);
    if (keyObj && keyObj.fpr) {
      let keyGroupBox = buildKeyGroupBox(keyObj);
      keyGroupBox.getElementsByClassName("annealmailKeyImportDetails")[0].addEventListener('click', onClickFunc, true);
  ...
  if (keys.length) {
    ...
    AnnealMailDialog.longAlert(window, AnnealMailData.convertCcrToUnicode(AnnealMailLocale.getString("importInfoNoKeys")));
    AnnealMailEvents.dispatchEvent(window.close, 0);
    return;
  }

  AnnealMailEvents.dispatchEvent(resizeDlg, 0);
}
...
function buildKeyGroupBox(keyObj) {
  ...
  userid.setAttribute("class", "annealmailKeyImportUserId");
  caption.setAttribute("label", AnnealMailLocale.getString("importInfoSuccess"));
  caption.setAttribute("class", "annealmailKeyImportCaption");
  infoLabelH1.setAttribute("value", AnnealMailLocale.getString("importInfoBits"));
  infoLabelH2.setAttribute("value", AnnealMailLocale.getString("importInfoCreated"));
  infoLabelH3.setAttribute("value", "");
  infoLabelB1.setAttribute("value", keyObj.keySize);
  infoLabelB2.setAttribute("value", keyObj.created);
  infoLabelB3.setAttribute("value", AnnealMailLocale.getString("importInfoDetails"));
  infoLabelB3.setAttribute("keyid", keyObj.keyId);
  infoLabelB3.setAttribute("class", "annealmailKeyImportDetails");

  infoRowHead.appendChild(infoLabelH1);
  infoRowHead.appendChild(infoLabelH2);
  infoRowHead.appendChild(infoLabelH3);
  infoRowHead.setAttribute("class", "annealmailKeyImportHeader");
  ...
  fprLabel.setAttribute("value", AnnealMailLocale.getString("importInfoFpr"));
  fprLabel.setAttribute("class", "annealmailKeyImportHeader");
...
function centerDialog() {
  if (AnnealMailOS.getOS() != "Darwin")
    document.getElementById("annealmailKeyImportInfo").centerWindowOnScreen();
}
