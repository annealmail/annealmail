/*global Components: false, AnnealMailLocale: false, AnnealMailApp: false, Dialog: false, AnnealMailTimer: false */
/*
...
Components.utils.import("resource://annealmail/glodaMime.jsm");
...
Components.utils.import("resource://annealmail/armor.jsm"); /*global AnnealMailArmor: false */
...
AnnealMail.msg = {
  ...
  encryptByRules: AnnealMailConstants.ENIG_UNDEF,
  signByRules: AnnealMailConstants.ENIG_UNDEF,
  pgpmimeByRules: AnnealMailConstants.ENIG_UNDEF,

  // forced to encrypt/sign/pgpmime?
  // (1:ENIG_UNDEF(undef/maybe), 0:ENIG_NEVER(never/forceNo), 2:ENIG_ALWAYS(always/forceYes))
  encryptForced: AnnealMailConstants.ENIG_UNDEF,
  signForced: AnnealMailConstants.ENIG_UNDEF,
  pgpmimeForced: AnnealMailConstants.ENIG_UNDEF,
  ...
  statusEncrypted: AnnealMailConstants.ENIG_FINAL_UNDEF,
  statusSigned: AnnealMailConstants.ENIG_FINAL_UNDEF,
  statusPGPMime: AnnealMailConstants.ENIG_FINAL_UNDEF,
  ...
  compFieldsEnig_CID: "@mozdev.org/annealmail/composefields;1",

  saveDraftError: 0,


  composeStartup: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.composeStartup\n");

    function delayedProcessFinalState() {
      AnnealMailTimer.setTimeout(function _f() {
          AnnealMail.msg.processFinalState();
    ...
    function addSecurityListener(itemId, func) {
      ...
      if (s) {
        ...
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: addSecurityListener - cannot find element " + itemId + "\n");
      }
    }
    ...
    if (msgId) {
      msgId.addEventListener("command", AnnealMail.msg.setIdentityCallback);
    }

    var subj = document.getElementById("msgSubject");
    subj.addEventListener('focus', AnnealMail.msg.fireSendFlags);
    //subj.setAttribute('onfocus', "AnnealMail.msg.fireSendFlags()");
  ...
  composeUnload: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.composeUnload\n");
    //if (gMsgCompose)
    //  gMsgCompose.UnregisterStateListener(AnnealMail.composeStateListener);
  },


  handleClick: function(event, modifyType) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.handleClick\n");
    switch (event.button) {
      case 2:
  ...
  setIdentityCallback: function(elementId) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.setIdentityCallback: elementId=" + elementId + "\n");

    AnnealMailTimer.setTimeout(function _f() {
        AnnealMail.msg.setIdentityDefaults();
      },
      50);
  },
  ...
  getAccDefault: function(key) {
    //AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.getAccDefault: identity="+this.identity.key+"("+this.identity.email+") key="+key+"\n");
    ...
    if (enabled) {
      ...
      //AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.getAccDefault:   "+key+"="+res+"\n");
      return res;
    }
    else {
    ...
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.getAccDefault:   internal error: invalid key '" + key + "'\n");
    return null;
  },


  setIdentityDefaults: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.setIdentityDefaults\n");

    this.identity = getCurrentIdentity();
    if (this.getAccDefault("enabled")) {
      AnnealMailFuncs.getSignMsg(this.identity); // convert old acc specific to new acc specific options
    }
    else {
      // reset status strings in menu to useful defaults
      this.statusEncryptedStr = AnnealMailLocale.getString("encryptNo");
      this.statusSignedStr = AnnealMailLocale.getString("signNo", [""]);
      this.statusPGPMimeStr = AnnealMailLocale.getString("pgpmimeYes");
      this.statusInlinePGPStr = AnnealMailLocale.getString("pgpmimeNo");
      this.statusAttachOwnKey = AnnealMailLocale.getString("attachOwnKeyNo");
    }
  ...
  processAccountSpecificDefaultOptions: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.processAccountSpecificDefaultOptions\n");

    const nsIAnnealMail = Components.interfaces.nsIAnnealMail;
    const SIGN = nsIAnnealMail.SEND_SIGNED;
    const ENCRYPT = nsIAnnealMail.SEND_ENCRYPTED;
    ...
    if (this.getAccDefault("encrypt")) {
      ...
      this.reasonEncrypted = AnnealMailLocale.getString("reasonEnabledByDefault");
    }
    if (this.getAccDefault("sign")) {
      this.sendMode |= SIGN;
      this.reasonSigned = AnnealMailLocale.getString("reasonEnabledByDefault");
    }
  ...
  getMsgProperties: function(msgUri, draft) {
    AnnealMailLog.DEBUG("annealmailMessengerOverlay.js: AnnealMail.msg.getMsgProperties:\n");
    const nsIAnnealMail = Components.interfaces.nsIAnnealMail;
    ...
    try {
      ...
      if (msgHdr) {
        properties = msgHdr.getUint32Property("annealmail");
        if (draft) {
          try {
            ...
            AnnealMailLog.DEBUG("annealmailMessengerOverlay.js: AnnealMail.msg.getMsgProperties: cannot use msgHdrToMimeMessage\n");
          }
        }
      }
    }
    catch (ex) {
      AnnealMailLog.DEBUG("annealmailMessengerOverlay.js: AnnealMail.msg.getMsgProperties: got exception '" + ex.toString() + "'\n");
    }

    if (AnnealMailURIs.isEncryptedUri(msgUri)) {
      properties |= nsIAnnealMail.DECRYPTION_OKAY;
    }
  ...
  getMsgPropertiesCb: function(msg, mimeMsg) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.getMsgPropertiesCb\n");

    const nsIAnnealMail = Components.interfaces.nsIAnnealMail;

    var stat = "";
    if (mimeMsg && mimeMsg.headers["x-annealmail-draft-status"]) {
      stat = String(mimeMsg.headers["x-annealmail-draft-status"]);
    }
    else {
      return;
    }

    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.getMsgPropertiesCb: draftStatus: " + stat + "\n");

    if (stat.substr(0, 1) == "N") {
      // new style drafts (AnnealMail 1.7)

      var enc = "final-encryptDefault";
      switch (Number(stat.substr(1, 1))) {
        case AnnealMailConstants.ENIG_NEVER:
          enc = "final-encryptNo";
          break;
        case AnnealMailConstants.ENIG_ALWAYS:
      ...
      AnnealMail.msg.setFinalSendMode(enc);
      AnnealMail.msg.setFinalSendMode(sig);
      AnnealMail.msg.setFinalSendMode(pgpMime);

      if (stat.substr(4, 1) == "1") AnnealMail.msg.attachOwnKeyObj.appendAttachment = true;
    }
    else {
      // drafts from older versions of AnnealMail
      var flags = Number(stat);
      if (flags & nsIAnnealMail.SEND_SIGNED) AnnealMail.msg.setFinalSendMode('final-signYes');
      if (flags & nsIAnnealMail.SEND_ENCRYPTED) AnnealMail.msg.setFinalSendMode('final-encryptYes');
      if (flags & nsIAnnealMail.SEND_ATTACHMENT) AnnealMail.msg.attachOwnKeyObj.appendAttachment = true;
    }
    AnnealMail.msg.setOwnKeyStatus();
  },


  composeOpen: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.composeOpen\n");
    ...
    if (adrCol) {
      ...
      adrCol.setAttribute("oninput", attr + "; AnnealMail.msg.addressOnChange();");
      attr = adrCol.getAttribute("onchange");
      adrCol.setAttribute("onchange", attr + "; AnnealMail.msg.addressOnChange();");
      adrCol.setAttribute("observes", "annealmail-bc-sendprocess");
    }
    adrCol = document.getElementById("addressCol1#1"); // to/cc/bcc/... field
    if (adrCol) {
      let attr = adrCol.getAttribute("oncommand");
      adrCol.setAttribute("oncommand", attr + "; AnnealMail.msg.addressOnChange();");
      adrCol.setAttribute("observes", "annealmail-bc-sendprocess");
    }

    var draftId = gMsgCompose.compFields.draftId;

    if (AnnealMailPrefs.getPref("keepSettingsForReply") && (!(this.sendMode & ENCRYPT)) || (typeof(draftId) == "string" && draftId.length > 0)) {
      ...
      if (msgUri) {
        ...
        if (!msgIsDraft) {
          if (msgFlags & nsIAnnealMail.DECRYPTION_OKAY) {
            AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.composeOpen: has encrypted originalMsgUri\n");
            AnnealMailLog.DEBUG("originalMsgURI=" + gMsgCompose.originalMsgURI + "\n");
          ...
          else if (msgFlags & (nsIAnnealMail.GOOD_SIGNATURE |
  ...
  msgComposeReopen: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.msgComposeReopen\n");
    this.msgComposeReset(false); // false => not closing => call setIdentityDefaults()
    this.composeOpen();
    this.fireSendFlags();

    AnnealMailTimer.setTimeout(function _f() {
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay: re-determine send flags\n");
      try {
        ...
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay: re-determine send flags - ERROR: " + ex.toString() + "\n");
      }
    }.bind(AnnealMail.msg), 1000);
  },


  msgComposeClose: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.msgComposeClose\n");
    ...
    try {
      ...
      if (this.modifiedAttach) {
        ...
        for (var i in this.modifiedAttach) {
          if (this.modifiedAttach[i].origTemp) {
            AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.msgComposeClose: deleting " + this.modifiedAttach[i].origUrl + "\n");
      ...
      AnnealMailLog.ERROR("annealmailMsgComposeOverlay.js: ECSL.ComposeProcessDone: could not delete all files:\n" + ex.toString() + "\n");
    }
  ...
  msgComposeReset: function(closing) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.msgComposeReset\n");
    ...
    this.reasonSigned = "";
    this.encryptByRules = AnnealMailConstants.ENIG_UNDEF;
    this.signByRules = AnnealMailConstants.ENIG_UNDEF;
    this.pgpmimeByRules = AnnealMailConstants.ENIG_UNDEF;
    this.signForced = AnnealMailConstants.ENIG_UNDEF;
    this.encryptForced = AnnealMailConstants.ENIG_UNDEF;
    this.pgpmimeForced = AnnealMailConstants.ENIG_UNDEF;
    this.finalSignDependsOnEncrypt = false;
    this.statusSigned = AnnealMailConstants.ENIG_FINAL_UNDEF;
    this.statusEncrypted = AnnealMailConstants.ENIG_FINAL_UNDEF;
    this.statusPGPMime = AnnealMailConstants.ENIG_FINAL_UNDEF;
  ...
  initRadioMenu: function(prefName, optionIds) {
    AnnealMailLog.DEBUG("annealmailMessengerOverlay.js: AnnealMail.msg.initRadioMenu: " + prefName + "\n");

    var encryptId;

    var prefValue = AnnealMailPrefs.getPref(prefName);

    if (prefValue >= optionIds.length)
      return;

    var menuItem = document.getElementById("annealmail_" + optionIds[prefValue]);
    if (menuItem)
  ...
  usePpgMimeOption: function(value) {
    AnnealMailLog.DEBUG("annealmailMessengerOverlay.js: AnnealMail.msg.usePpgMimeOption: " + value + "\n");

    AnnealMailPrefs.setPref("usePGPMimeOption", value);

    return true;
  },

  togglePgpMime: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.togglePgpMime\n");
  ...
  toggleAttachOwnKey: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.toggleAttachOwnKey\n");
    AnnealMailCore.getService(window); // make sure AnnealMail is loaded and working
  ...
  toggleProtectHeaders: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.toggleProtectHeaders\n");
    AnnealMailCore.getService(window); // make sure AnnealMail is loaded and working
  ...
  displayProtectHeadersStatus: function() {
    let bc = document.getElementById("annealmail-bc-protectHdr");
  ...
  setOwnKeyStatus: function() {
    let bc = document.getElementById("annealmail-bc-attach");
    let attachIcon = document.getElementById("button-annealmail-attach");

    if (this.allowAttachOwnKey() === 0) {
      this.statusAttachOwnKey = AnnealMailLocale.getString("attachOwnKeyDisabled");
    }
    else {
      if (this.attachOwnKeyObj.appendAttachment) {
        bc.setAttribute("addPubkey", "true");
        bc.setAttribute("checked", "true");
        this.statusAttachOwnKey = AnnealMailLocale.getString("attachOwnKeyYes");
      }
      else {
  ...
  attachOwnKey: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.attachOwnKey:\n");
    ...
    if (this.identity.getIntAttribute("pgpKeyMode") > 0) {
      ...
      AnnealMailLog.ERROR("annealmailMsgComposeOverlay.js: AnnealMail.msg.attachOwnKey: trying to attach unknown own key!\n");
    }
  },

  attachKey: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.attachKey: \n");

    var resultObj = {};
    var inputObj = {};
    inputObj.dialogHeader = AnnealMailLocale.getString("keysToExport");
  ...
  extractAndAttachKey: function(uid) {
    ...
    try {
      ...
      AnnealMailLog.writeException("annealmailMsgComposeOverlay.js: AnnealMail.msg.extractAndAttachKey", ex);
    }
    ...
    AnnealMailKeyRing.extractKey(false, uid.join(" "), tmpFile /*.path */ , exitCodeObj, errorMsgObj);
  ...
  enableUndoEncryption: function(newStatus) {
    let eue = document.getElementById("annealmail_undo_encryption");
  ...
  undoEncryption: function(useEditorUndo) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.undoEncryption:\n");
    if (this.processed) {
      if (useEditorUndo) {
        AnnealMailTimer.setTimeout(function _f() {
          AnnealMail.msg.editor.undo(useEditorUndo);
        }, 10);
      }
      else {
  ...
  removeAttachedKey: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.removeAttachedKey: \n");
  ...
  replaceEditorText: function(text) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.replaceEditorText:\n");
    ...
    if (this.editor.textLength > 0) {
      this.editorInsertText("AnnealMail");
    }
    else {
  ...
  getMsgFolderFromUri: function(uri, checkFolderAttributes) {
    ...
    try {
      ...
      //AnnealMailLog.DEBUG("failed to get the folder resource\n");
    }
    return msgfolder;
  },
  ...
  goAccountManager: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.goAccountManager:\n");
    ...
    window.openDialog("chrome://annealmail/content/am-enigprefs-edit.xul", "", "dialog,modal,centerscreen", {
      identity: currentId,
      account: server
    });
    this.setIdentityDefaults();
  ...
  doPgpButton: function(what) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.doPgpButton: what=" + what + "\n");
    ...
    this.nextCommandId = "";
    AnnealMailCore.getService(window); // try to access AnnealMail to launch the wizard if needed

    // ignore settings for this account?
    try {
      if (!this.getAccDefault("enabled")) {
        if (AnnealMailDialog.confirmDlg(window, AnnealMailLocale.getString("configureNow"),
            AnnealMailLocale.getString("msgCompose.button.configure"))) {
  ...
  nextCommand: function(what) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.nextCommand: what=" + what + "\n");
  ...
  setSendMode: function(sendMode) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.setSendMode: sendMode=" + sendMode + "\n");
    const nsIAnnealMail = Components.interfaces.nsIAnnealMail;
    const SIGN = nsIAnnealMail.SEND_SIGNED;
    const ENCRYPT = nsIAnnealMail.SEND_ENCRYPTED;
    ...
    switch (sendMode) {
      ...
      case 'toggle-encrypt':
        ...
        break;
      default:
        AnnealMailDialog.alert(window, "AnnealMail.msg.setSendMode - unexpected value: " + sendMode);
        break;
    }
    // sendMode changed ?
  ...
  setFinalSendMode: function(sendMode) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.setFinalSendMode: sendMode=" + sendMode + "\n");
    ...
    switch (sendMode) {
      ...
      case 'final-encryptDefault':
        ...
        if (this.encryptForced != AnnealMailConstants.ENIG_UNDEF) { // if encrypt/noencrypt forced
          this.encryptForced = AnnealMailConstants.ENIG_UNDEF; // back to defaults/rules
        }
        break;
      case 'final-encryptYes':
        // switch encryption to "force encryption"
        if (this.encryptForced != AnnealMailConstants.ENIG_ALWAYS) { // if not forced to encrypt
          this.encryptForced = AnnealMailConstants.ENIG_ALWAYS; // force to encrypt
        }
        break;
      case 'final-encryptNo':
        // switch encryption to "force no to encrypt"
        if (this.encryptForced != AnnealMailConstants.ENIG_NEVER) { // if not forced not to encrypt
          this.encryptForced = AnnealMailConstants.ENIG_NEVER; // force not to encrypt
        }
        break;

      case 'final-signDefault':
        // switch signing to "use defaults & rules"
        if (this.signForced != AnnealMailConstants.ENIG_UNDEF) { // if sign/nosign forced
          // re-init if signing depends on encryption if this was broken before
          this.finalSignDependsOnEncrypt = (this.getAccDefault("signIfEnc") || this.getAccDefault("signIfNotEnc"));
          this.signForced = AnnealMailConstants.ENIG_UNDEF; // back to defaults/rules
        }
        break;
      case 'final-signYes':
        if (this.signForced != AnnealMailConstants.ENIG_ALWAYS) { // if not forced to sign
          this.signingNoLongerDependsOnEnc();
          this.signForced = AnnealMailConstants.ENIG_ALWAYS; // force to sign
        }
        break;
      case 'final-signNo':
        if (this.signForced != AnnealMailConstants.ENIG_NEVER) { // if not forced not to sign
          this.signingNoLongerDependsOnEnc();
          this.signForced = AnnealMailConstants.ENIG_NEVER; // force not to sign
        }
        break;

      case 'final-pgpmimeDefault':
        if (this.pgpmimeForced != AnnealMailConstants.ENIG_UNDEF) { // if any PGP mode forced
          this.pgpmimeForced = AnnealMailConstants.ENIG_UNDEF; // back to defaults/rules
        }
        break;
      case 'final-pgpmimeYes':
        if (this.pgpmimeForced != AnnealMailConstants.ENIG_ALWAYS) { // if not forced to PGP/Mime
          this.pgpmimeForced = AnnealMailConstants.ENIG_ALWAYS; // force to PGP/Mime
        }
        break;
      case 'final-pgpmimeNo':
        if (this.pgpmimeForced != AnnealMailConstants.ENIG_NEVER) { // if not forced not to PGP/Mime
          this.pgpmimeForced = AnnealMailConstants.ENIG_NEVER; // force not to PGP/Mime
        }
        break;
      ...
      case 'toggle-final-sign':
        ...
        switch (this.statusSigned) {
          case AnnealMailConstants.ENIG_FINAL_NO:
          case AnnealMailConstants.ENIG_FINAL_FORCENO:
            this.signForced = AnnealMailConstants.ENIG_ALWAYS; // force to sign
            break;
          ...
          case AnnealMailConstants.ENIG_FINAL_FORCEYES:
            this.signForced = AnnealMailConstants.ENIG_NEVER; // force not to sign
            break;
          case AnnealMailConstants.ENIG_FINAL_CONFLICT:
            this.signForced = AnnealMailConstants.ENIG_NEVER;
            break;
        }
        break;
      ...
      case 'toggle-final-encrypt':
        switch (this.statusEncrypted) {
          ...
          case AnnealMailConstants.ENIG_FINAL_FORCEYES:
            ...
            break;
          case AnnealMailConstants.ENIG_FINAL_CONFLICT:
            this.encryptForced = AnnealMailConstants.ENIG_NEVER;
            break;
        }
        break;
      ...
      case 'toggle-final-mime':
        switch (this.statusPGPMime) {
          case AnnealMailConstants.ENIG_FINAL_NO:
          case AnnealMailConstants.ENIG_FINAL_FORCENO:
            this.pgpmimeForced = AnnealMailConstants.ENIG_ALWAYS; // force PGP/MIME
            break;
          case AnnealMailConstants.ENIG_FINAL_YES:
          case AnnealMailConstants.ENIG_FINAL_FORCEYES:
            this.pgpmimeForced = AnnealMailConstants.ENIG_NEVER; // force Inline-PGP
            break;
          case AnnealMailConstants.ENIG_FINAL_CONFLICT:
            this.pgpmimeForced = AnnealMailConstants.ENIG_NEVER;
            break;
        }
        break;

      default:
        AnnealMailDialog.alert(window, "AnnealMail.msg.setFinalSendMode - unexpected value: " + sendMode);
        break;
    }
  ...
  processFinalState: function(sendFlags) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.processFinalState()\n");
    const nsIAnnealMail = Components.interfaces.nsIAnnealMail;
    const SIGN = nsIAnnealMail.SEND_SIGNED;
    const ENCRYPT = nsIAnnealMail.SEND_ENCRYPTED;
    ...
    if (this.encryptForced == AnnealMailConstants.ENIG_NEVER) { // force not to encrypt?
      encFinally = AnnealMailConstants.ENIG_FINAL_FORCENO;
      encReason = AnnealMailLocale.getString("reasonManuallyForced");
    }
    else if (this.encryptForced == AnnealMailConstants.ENIG_ALWAYS) { // force to encrypt?
      encFinally = AnnealMailConstants.ENIG_FINAL_FORCEYES;
      encReason = AnnealMailLocale.getString("reasonManuallyForced");
    }
    else switch (this.encryptByRules) {
      case AnnealMailConstants.ENIG_NEVER:
        encFinally = AnnealMailConstants.ENIG_FINAL_NO;
        encReason = AnnealMailLocale.getString("reasonByRecipientRules");
        break;
      case AnnealMailConstants.ENIG_UNDEF:
        if (this.sendMode & ENCRYPT) {
          encFinally = AnnealMailConstants.ENIG_FINAL_YES;
          if (this.getAccDefault("encrypt")) {
            encReason = AnnealMailLocale.getString("reasonEnabledByDefault");
          }
        }
        else {
          encFinally = AnnealMailConstants.ENIG_FINAL_NO;
      ...
      case AnnealMailConstants.ENIG_ALWAYS:
        ...
        encReason = AnnealMailLocale.getString("reasonByRecipientRules");
        break;
      case AnnealMailConstants.ENIG_AUTO_ALWAYS:
        encFinally = AnnealMailConstants.ENIG_FINAL_YES;
        encReason = AnnealMailLocale.getString("reasonByAutoEncryption");
        break;
      case AnnealMailConstants.ENIG_CONFLICT:
        encFinally = AnnealMailConstants.ENIG_FINAL_CONFLICT;
        encReason = AnnealMailLocale.getString("reasonByConflict");
        break;
    }
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js:   encrypt=" + ((this.sendMode & ENCRYPT) !== 0) + " encryptByRules=" + this.encryptByRules + " encFinally=" + encFinally + "\n");
    AnnealMailLog.DEBUG("                                encReason=" + encReason + "\n");

    // process resulting sign mode
    if (this.signForced == AnnealMailConstants.ENIG_NEVER) { // force not to sign?
      signFinally = AnnealMailConstants.ENIG_FINAL_FORCENO;
      signReason = AnnealMailLocale.getString("reasonManuallyForced");
    }
    else if (this.signForced == AnnealMailConstants.ENIG_ALWAYS) { // force to sign?
      signFinally = AnnealMailConstants.ENIG_FINAL_FORCEYES;
      signReason = AnnealMailLocale.getString("reasonManuallyForced");
    }
    else switch (this.signByRules) {
      case AnnealMailConstants.ENIG_NEVER:
        signFinally = AnnealMailConstants.ENIG_FINAL_NO;
        signReason = AnnealMailLocale.getString("reasonByRecipientRules");
        break;
      case AnnealMailConstants.ENIG_UNDEF:
        if (this.sendMode & SIGN) {
          signFinally = AnnealMailConstants.ENIG_FINAL_YES;
          if (this.getAccDefault("sign")) {
            signReason = AnnealMailLocale.getString("reasonEnabledByDefault");
          }
        }
        else {
          signFinally = AnnealMailConstants.ENIG_FINAL_NO;
      ...
      case AnnealMailConstants.ENIG_ALWAYS:
        ...
        break;
      case AnnealMailConstants.ENIG_CONFLICT:
        signFinally = AnnealMailConstants.ENIG_FINAL_CONFLICT;
        signReason = AnnealMailLocale.getString("reasonByConflict");
        break;
    }
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js:   signed=" + ((this.sendMode & SIGN) !== 0) + " signByRules=" + this.signByRules + " signFinally=" + signFinally + "\n");
    AnnealMailLog.DEBUG("                                signReason=" + signReason + "\n");
    ...
    if (this.finalSignDependsOnEncrypt) {
      if (this.signByRules == AnnealMailConstants.ENIG_UNDEF) { // if final sign mode not clear yet
        ...
        switch (encFinally) {
          ...
          case AnnealMailConstants.ENIG_FINAL_FORCEYES:
            if (this.getAccDefault("signIfEnc")) {
              signFinally = AnnealMailConstants.ENIG_FINAL_YES;
              signReason = AnnealMailLocale.getString("reasonByEncryptionMode");
            }
            break;
          case AnnealMailConstants.ENIG_FINAL_NO:
          case AnnealMailConstants.ENIG_FINAL_FORCENO:
        ...
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js:   derived signFinally=" + signFinally + "\n");
        AnnealMailLog.DEBUG("                                signReason=" + signReason + "\n");
      }
    }
    ...
    if (gSMFields) {
      if (gSMFields.signMessage || gSMFields.requireEncryptMessage) {
        if (AnnealMailPrefs.getPref("mimePreferPgp") == 2) {
          encFinally = AnnealMailConstants.ENIG_FINAL_SMIME_DISABLED;
          encReason = AnnealMailLocale.getString("reasonSmimeConflict");
          signFinally = AnnealMailConstants.ENIG_FINAL_SMIME_DISABLED;
          signReason = AnnealMailLocale.getString("reasonSmimeConflict");
    ...
    if (this.pgpmimeForced == AnnealMailConstants.ENIG_NEVER) { // force not to PGP/Mime?
      pgpmimeFinally = AnnealMailConstants.ENIG_FINAL_FORCENO;
    }
    else if (this.pgpmimeForced == AnnealMailConstants.ENIG_ALWAYS) { // force to PGP/Mime?
      pgpmimeFinally = AnnealMailConstants.ENIG_FINAL_FORCEYES;
    }
    else switch (this.pgpmimeByRules) {
      case AnnealMailConstants.ENIG_NEVER:
        pgpmimeFinally = AnnealMailConstants.ENIG_FINAL_NO;
        break;
      case AnnealMailConstants.ENIG_UNDEF:
        pgpmimeFinally = ((this.sendPgpMime || (this.sendMode & nsIAnnealMail.SEND_PGP_MIME)) ? AnnealMailConstants.ENIG_FINAL_YES : AnnealMailConstants.ENIG_FINAL_NO);
        break;
      case AnnealMailConstants.ENIG_ALWAYS:
        pgpmimeFinally = AnnealMailConstants.ENIG_FINAL_YES;
        break;
      case AnnealMailConstants.ENIG_CONFLICT:
        pgpmimeFinally = AnnealMailConstants.ENIG_FINAL_CONFLICT;
        break;
    }
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js:   pgpmimeByRules=" + this.pgpmimeByRules + " pgpmimeFinally=" + pgpmimeFinally + "\n");
  ...
  updateStatusBar: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.updateStatusBar()\n");
    ...
    var toolbarTxt = document.getElementById("annealmail-toolbar-text");
    var encBroadcaster = document.getElementById("annealmail-bc-encrypt");
    var signBroadcaster = document.getElementById("annealmail-bc-sign");
    var attachBroadcaster = document.getElementById("annealmail-bc-attach");

    // annealmail disabled for this identity?:
    if (!this.getAccDefault("enabled")) {
      // hide icons if annealmail not enabled
      ...
      if (toolbarTxt) {
        toolbarTxt.value = AnnealMailLocale.getString("msgCompose.toolbarTxt.disabled");
    ...
    switch (this.statusEncrypted) {
      case AnnealMailConstants.ENIG_FINAL_FORCENO:
        encSymbol = "forceNo";
        encStr = AnnealMailLocale.getString("encryptMessageNorm");
        break;
      case AnnealMailConstants.ENIG_FINAL_FORCEYES:
        doEncrypt = true;
        encSymbol = "forceYes";
      ...
      case AnnealMailConstants.ENIG_FINAL_NO:
        ...
        encStr = AnnealMailLocale.getString("encryptMessageAuto");
        break;
      case AnnealMailConstants.ENIG_FINAL_YES:
        doEncrypt = true;
    ...
    if (doEncrypt) {
      if (this.reasonEncrypted && this.reasonEncrypted !== "") {
        encReasonStr = AnnealMailLocale.getString("encryptOnWithReason", [this.reasonEncrypted]);
      }
      else {
        encReasonStr = AnnealMailLocale.getString("encryptOn");
      }
    }
    else {
      if (this.reasonEncrypted && this.reasonEncrypted !== "") {
        encReasonStr = AnnealMailLocale.getString("encryptOffWithReason", [this.reasonEncrypted]);
      }
      else {
        encReasonStr = AnnealMailLocale.getString("encryptOff");
      }
    }
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js:   encSymbol=" + encSymbol + "  encReasonStr=" + encReasonStr + "\n");

    // update encrypt icon and tooltip/menu-text
    encBroadcaster.setAttribute("encrypted", encSymbol);
    var encIcon = document.getElementById("button-annealmail-encrypt");
    if (encIcon) {
      encIcon.setAttribute("tooltiptext", encReasonStr);
    }
    this.statusEncryptedStr = encStr;
    this.setChecked("annealmail-bc-encrypt", doEncrypt);
    ...
    switch (this.statusSigned) {
      case AnnealMailConstants.ENIG_FINAL_FORCENO:
        signSymbol = "forceNo";
        signStr = AnnealMailLocale.getString("signMessageNorm");
        signReasonStr = AnnealMailLocale.getString("signOffWithReason", [this.reasonSigned]);
        break;
      case AnnealMailConstants.ENIG_FINAL_FORCEYES:
        doSign = true;
        signSymbol = "forceYes";
        signStr = AnnealMailLocale.getString("signMessageNorm");
        signReasonStr = AnnealMailLocale.getString("signOnWithReason", [this.reasonSigned]);
        break;
      case AnnealMailConstants.ENIG_FINAL_NO:
        signSymbol = "inactiveNone";
        signStr = AnnealMailLocale.getString("signMessageAuto");
        signReasonStr = AnnealMailLocale.getString("signOffWithReason", [this.reasonSigned]);
        break;
      case AnnealMailConstants.ENIG_FINAL_YES:
        doSign = true;
    ...
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js:   signSymbol=" + signSymbol + "  signReasonStr=" + signReasonStr + "\n");

    // update sign icon and tooltip/menu-text
    signBroadcaster.setAttribute("signed", signSymbol);
    var signIcon = document.getElementById("button-annealmail-sign");
    if (signIcon) {
      signIcon.setAttribute("tooltiptext", signReasonStr);
    }
    this.statusSignedStr = signStr;
    this.setChecked("annealmail-bc-sign", doSign);

    // process resulting toolbar message
    var toolbarMsg = "";
    if (doSign && doEncrypt) {
      toolbarMsg = AnnealMailLocale.getString("msgCompose.toolbarTxt.signAndEncrypt");
    }
    else if (doSign) {
      toolbarMsg = AnnealMailLocale.getString("msgCompose.toolbarTxt.signOnly");
    }
    else if (doEncrypt) {
      toolbarMsg = AnnealMailLocale.getString("msgCompose.toolbarTxt.encryptOnly");
    }
    else {
      toolbarMsg = AnnealMailLocale.getString("msgCompose.toolbarTxt.noEncryption");
    }
    ...
    if (gSMFields) {
      if (gSMFields.signMessage || gSMFields.requireEncryptMessage) {
        ...
        switch (AnnealMailPrefs.getPref("mimePreferPgp")) {
          case 0:
            // prefer OpenPGP over S/MIME
            if (doSign || doEncrypt) {
              toolbarMsg += " " + AnnealMailLocale.getString("msgCompose.toolbarTxt.smimeOff");
            }
            break;
          case 1:
            // ask user
            if (doSign || doEncrypt) {
              toolbarMsg += " " + AnnealMailLocale.getString("msgCompose.toolbarTxt.smime");
            }
            if (doEncryptDrafts) {
              toolbarMsg += " " + AnnealMailLocale.getString("msgCompose.toolbarTxt.smimeNoDraftEncryption");
            }
            break;
          case 2:
            ...
            encBroadcaster.setAttribute("disabled", "true");
            signBroadcaster.setAttribute("disabled", "true");
            toolbarMsg = AnnealMailLocale.getString("msgCompose.toolbarTxt.smimeSignOrEncrypt");
            if (doEncryptDrafts) {
              toolbarMsg += " " + AnnealMailLocale.getString("msgCompose.toolbarTxt.smimeNoDraftEncryption");
            }
            break;
    ...
    if (toolbarTxt) {
      ...
      if (gMsgCompose.compFields.securityInfo) {
        ...
        if (!doSign && !doEncrypt &&
          ...
          toolbarTxt.setAttribute("class", "annealmailStrong");
        }
        else {
          toolbarTxt.removeAttribute("class");
        }
    ...
    if (this.statusPGPMime == AnnealMailConstants.ENIG_FINAL_YES) {
      this.statusPGPMimeStr = AnnealMailLocale.getString("pgpmimeAuto");
    }
    else {
      this.statusPGPMimeStr = AnnealMailLocale.getString("pgpmimeNormal");
    }

    if (this.statusPGPMime == AnnealMailConstants.ENIG_FINAL_NO) {
      this.inlinePGPStr = AnnealMailLocale.getString("inlinePGPAuto");
    }
    else {
      this.inlinePGPStr = AnnealMailLocale.getString("inlinePGPNormal");
    }
   ...
   * @result: Number:
   *          -1: account not enabled for AnnealMail
  ...
  determineSendFlags: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.focusChange: AnnealMail.msg.determineSendFlags\n");
    ...
    if (this.getAccDefault("enabled")) {
      ...
      if (!AnnealMail.msg.composeBodyReady) {
      ...
      // process rules
      if (toAddrList.length > 0 && AnnealMailPrefs.getPref("assignKeysByRules")) {
        var matchedKeysObj = {};
        var flagsObj = {};
        if (AnnealMailRules.mapAddrsToKeys(toAddrList.join(", "),
      ...
      if (toAddrList.length > 0 && this.encryptByRules == AnnealMailConstants.ENIG_UNDEF && AnnealMailPrefs.getPref("autoSendEncrypted") == 1) {
        var validKeyList = AnnealMail.hlp.validKeysForAllRecipients(toAddrList.join(", "));
        if (validKeyList) {
          this.encryptByRules = AnnealMailConstants.ENIG_AUTO_ALWAYS;
        }
      }
    }
  ...
  setMenuSettings: function(postfix) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.setMenuSettings: postfix=" + postfix + "\n");

    const nsIAnnealMail = Components.interfaces.nsIAnnealMail;
    const SIGN = nsIAnnealMail.SEND_SIGNED;
    const ENCRYPT = nsIAnnealMail.SEND_ENCRYPTED;

    var elem = document.getElementById("annealmail_compose_sign_item" + postfix);
    if (elem) {
    ...
    elem = document.getElementById("annealmail_compose_encrypt_item" + postfix);
    if (elem) {
    ...
    elem = document.getElementById("annealmail_compose_pgpmime_item" + postfix);
    if (elem) {
    ...
    elem = document.getElementById("annealmail_compose_inline_item" + postfix);
    if (elem) {
      ...
      switch (this.statusPGPMime) {
        ...
        case AnnealMailConstants.ENIG_FINAL_FORCENO:
        case AnnealMailConstants.ENIG_FINAL_CONFLICT:
        case AnnealMailConstants.ENIG_FINAL_UNDEF:
          elem.setAttribute("checked", "true");
          break;
        default:
          elem.setAttribute("checked", "false");
    ...
    /*
        this.setChecked("annealmail_final_encryptDefault"+postfix, this.encryptForced == AnnealMailConstants.ENIG_UNDEF);
        this.setChecked("annealmail_final_encryptYes"+postfix, this.encryptForced == AnnealMailConstants.ENIG_ALWAYS);
        this.setChecked("annealmail_final_encryptNo"+postfix, this.encryptForced == AnnealMailConstants.ENIG_NEVER);
        this.setChecked("annealmail_final_signDefault"+postfix, this.signForced == AnnealMailConstants.ENIG_UNDEF);
        this.setChecked("annealmail_final_signYes"+postfix, this.signForced == AnnealMailConstants.ENIG_ALWAYS);
        this.setChecked("annealmail_final_signNo"+postfix, this.signForced == AnnealMailConstants.ENIG_NEVER);
        this.setChecked("annealmail_final_pgpmimeDefault"+postfix, this.pgpmimeForced == AnnealMailConstants.ENIG_UNDEF);
        this.setChecked("annealmail_final_pgpmimeYes"+postfix, this.pgpmimeForced == AnnealMailConstants.ENIG_ALWAYS);
        this.setChecked("annealmail_final_pgpmimeNo"+postfix, this.pgpmimeForced == AnnealMailConstants.ENIG_NEVER);
    */

    let menuElement = document.getElementById("annealmail_insert_own_key");
    if (menuElement) {
      if (this.identity.getIntAttribute("pgpKeyMode") > 0) {
  ...
  displaySecuritySettings: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.displaySecuritySettings\n");

    if (this.statusEncrypted == AnnealMailConstants.ENIG_FINAL_SMIME_DISABLED) {
      AnnealMailDialog.alert(window, AnnealMailLocale.getString("msgCompose.toolbarTxt.smimeConflict"));
      return;
    }
    ...
    window.openDialog("chrome://annealmail/content/annealmailEncryptionDlg.xul", "", "dialog,modal,centerscreen", inputObj);
  ...
  signingNoLongerDependsOnEnc: function() {
    if (this.finalSignDependsOnEncrypt) {
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.signingNoLongerDependsOnEnc(): unbundle final signing\n");
      this.finalSignDependsOnEncrypt = false;

      AnnealMailDialog.alertPref(window, AnnealMailLocale.getString("signIconClicked"), "displaySignWarn");
    }
  },


  confirmBeforeSend: function(toAddrStr, ccrKeys, sendFlags, isOffline) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.confirmBeforeSend: sendFlags=" + sendFlags + "\n");
    ...
    if (sendFlags & (ENCRYPT | SIGN)) {
      if (sendFlags & nsIAnnealMail.SEND_PGP_MIME) {
        msgStatus += " " + AnnealMailLocale.getString("statPGPMIME");
      }
      if (sendFlags & SIGN) {
        msgStatus += " " + AnnealMailLocale.getString("statSigned");
      }
      if (sendFlags & ENCRYPT) {
        msgStatus += " " + AnnealMailLocale.getString("statEncrypted");
      }
    }
    else {
      msgStatus += " " + AnnealMailLocale.getString("statPlain");
    }

    // create message
    var msgConfirm = "";
    if (isOffline || sendFlags & nsIAnnealMail.SEND_LATER) {
      msgConfirm = AnnealMailLocale.getString("offlineSave", [msgStatus, AnnealMailFuncs.stripEmail(toAddrStr).replace(/,/g, ", ")]);
    }
    else {
      msgConfirm = AnnealMailLocale.getString("onlineSend", [msgStatus, AnnealMailFuncs.stripEmail(toAddrStr).replace(/,/g, ", ")]);
    }

    // add list of keys
    if (sendFlags & ENCRYPT) {
      ccrKeys = ccrKeys.replace(/^, /, "").replace(/, $/, "");

      // make ccr keys unique
      let keyList = ccrKeys.split(/[, ]+/).reduce(function _f(p, key) {
        if (p.indexOf(key) < 0) p.push(key);
        return p;
      }, []);

      msgConfirm += "\n\n" + AnnealMailLocale.getString("encryptKeysNote", [keyList.join(", ")]);
    }

    return AnnealMailDialog.confirmDlg(window, msgConfirm,
      AnnealMailLocale.getString((isOffline || sendFlags & nsIAnnealMail.SEND_LATER) ?
  ...
  addRecipients: function(toAddrList, recList) {
    for (var i = 0; i < recList.length; i++) {
      toAddrList.push(AnnealMailFuncs.stripEmail(recList[i].replace(/[\",]/g, "")));
    }
  },

  setDraftStatus: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.setDraftStatus - enabling draft mode\n");
    ...
    this.setAdditionalHeader("X-AnnealMail-Draft-Status", draftStatus);
  },
  ...
  getSenderUserId: function() {
    ...
    if (this.identity.getIntAttribute("pgpKeyMode") > 0) {
      ...
      if (!userIdValue) {
        ...
        var mesg = AnnealMailLocale.getString("composeSpecifyEmail");

        var valueObj = {
          value: userIdValue
        };

        if (AnnealMailDialog.promptValue(window, mesg, valueObj)) {
    ...
    if (typeof(userIdValue) != "string") {
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.getSenderUserId: type of userIdValue=" + typeof(userIdValue) + "\n");
   ...
   */
  keySelection: function(annealmailSvc, sendFlags, optSendFlags, gotSendFlags, fromAddr, toAddrList, bccAddrList) {
    AnnealMailLog.DEBUG("=====> keySelection()\n");
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.keySelection()\n");
    const nsIAnnealMail = Components.interfaces.nsIAnnealMail;
    const SIGN = nsIAnnealMail.SEND_SIGNED;
    const ENCRYPT = nsIAnnealMail.SEND_ENCRYPTED;
    ...
    if (toAddrStr.length === 0) {
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.keySelection(): skip key selection because we neither have \"to\" nor \"cc\" addresses\n");

      if (this.statusPGPMime == AnnealMailConstants.ENIG_FINAL_YES ||
        this.statusPGPMime == AnnealMailConstants.ENIG_FINAL_FORCEYES) {
        sendFlags |= nsIAnnealMail.SEND_PGP_MIME;
      }
      else if (this.statusPGPMime == AnnealMailConstants.ENIG_FINAL_NO ||
        this.statusPGPMime == AnnealMailConstants.ENIG_FINAL_FORCENO ||
        this.statusPGPMime == AnnealMailConstants.ENIG_FINAL_CONFLICT) {
        sendFlags &= ~nsIAnnealMail.SEND_PGP_MIME;
      }
    ...
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.keySelection(): toAddrStr=\"" + toAddrStr + "\" bccAddrStr=\"" + bccAddrStr + "\"\n");
    ...
    if (AnnealMailPrefs.getPref("assignKeysByRules") &&
      !AnnealMailPrefs.getPref("assignKeysByEmailAddr") &&
      !AnnealMailPrefs.getPref("assignKeysManuallyIfMissing") &&
      !AnnealMailPrefs.getPref("assignKeysManuallyAlways")) {
    ...
    do {
      ...
      if (AnnealMailPrefs.getPref("assignKeysByRules") && this.enableRules) {
      ...
      if (sendFlags & ENCRYPT) {
        let result = this.encryptTestMessage(annealmailSvc, sendFlags, optSendFlags,
    ...
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.keySelection(): return toAddrStr=\"" + toAddrStr + "\" bccAddrStr=\"" + bccAddrStr + "\"\n");
    AnnealMailLog.DEBUG("  <=== keySelection()");
    return {
      sendFlags: sendFlags,
      toAddrStr: toAddrStr,
  ...
  preferPgpOverSmime: function(sendFlags) {
    ...
    if (gMsgCompose.compFields.securityInfo instanceof Components.interfaces.nsIMsgSMIMECompFields &&
      (sendFlags & (nsIAnnealMail.SEND_SIGNED | nsIAnnealMail.SEND_ENCRYPTED))) {

      let si = gMsgCompose.compFields.securityInfo.QueryInterface(Components.interfaces.nsIMsgSMIMECompFields);
      if (si.requireEncryptMessage || si.signMessage) {

        if (sendFlags & nsIAnnealMail.SAVE_MESSAGE) {
          // use S/MIME if it's enabled for saving drafts
          return false;
        }
        else {
          var promptSvc = AnnealMailDialog.getPromptSvc();
          var prefAlgo = AnnealMailPrefs.getPref("mimePreferPgp");
          if (prefAlgo == 1) {
            var checkedObj = {
              value: null
            };
            prefAlgo = promptSvc.confirmEx(window,
              ...
              AnnealMailLocale.getString("pgpMime_sMime.dlg.text"), (promptSvc.BUTTON_TITLE_IS_STRING * promptSvc.BUTTON_POS_0) +
              (promptSvc.BUTTON_TITLE_CANCEL * promptSvc.BUTTON_POS_1) +
              (promptSvc.BUTTON_TITLE_IS_STRING * promptSvc.BUTTON_POS_2),
              AnnealMailLocale.getString("pgpMime_sMime.dlg.pgpMime.button"),
              null,
              AnnealMailLocale.getString("pgpMime_sMime.dlg.sMime.button"),
              AnnealMailLocale.getString("dlgKeepSetting"),
              checkedObj);
            if (checkedObj.value && (prefAlgo === 0 || prefAlgo == 2)) AnnealMailPrefs.setPref("mimePreferPgp", prefAlgo);
          }
          switch (prefAlgo) {
            case 0:
  ...
  processRules: function(forceRecipientSettings, sendFlags, optSendFlags, toAddrStr, bccAddrStr) {
    AnnealMailLog.DEBUG("=====> processRules()\n");
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.processRules(): toAddrStr=\"" + toAddrStr + "\" bccAddrStr=\"" + bccAddrStr + "\" forceRecipientSettings=" +
    ...
    if (!AnnealMailRules.mapAddrsToKeys(toAddrStr,
    ...
    if (matchedKeysObj.value) {
      ...
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.processRules(): after mapAddrsToKeys() toAddrStr=\"" + toAddrStr + "\"\n");
    }
    this.encryptByRules = flagsObj.encrypt;
    this.signByRules = flagsObj.sign;
    this.pgpmimeByRules = flagsObj.pgpMime;
    ...
    if (toAddrStr.length > 0 && this.encryptByRules == AnnealMailConstants.ENIG_UNDEF && AnnealMailPrefs.getPref("autoSendEncrypted") == 1) {
      var validKeyList = AnnealMail.hlp.validKeysForAllRecipients(toAddrStr);
      if (validKeyList) {
        this.encryptByRules = AnnealMailConstants.ENIG_AUTO_ALWAYS;
    ...
    if (this.statusPGPMime == AnnealMailConstants.ENIG_FINAL_CONFLICT) {
      this.statusPGPMime = AnnealMailConstants.ENIG_FINAL_NO;
    }
    // - encrypt/sign conflicts result into result 0/'never'
    //   with possible dialog to give a corresponding feedback
    var conflictFound = false;
    if (this.statusEncrypted == AnnealMailConstants.ENIG_FINAL_CONFLICT) {
      this.statusEncrypted = AnnealMailConstants.ENIG_FINAL_NO;
      conflictFound = true;
    }
    if (this.statusSigned == AnnealMailConstants.ENIG_FINAL_CONFLICT) {
      this.statusSigned = AnnealMailConstants.ENIG_FINAL_NO;
      conflictFound = true;
    }
    if (conflictFound) {
      if (!AnnealMail.hlp.processConflicts(this.statusEncrypted == AnnealMailConstants.ENIG_FINAL_YES || this.statusEncrypted == AnnealMailConstants.ENIG_FINAL_FORCEYES,
          this.statusSigned == AnnealMailConstants.ENIG_FINAL_YES || this.statusSigned == AnnealMailConstants.ENIG_FINAL_FORCEYES)) {
        return null;
      }
    }
    ...
    if (!AnnealMailRules.mapAddrsToKeys(bccAddrStr,
        forceRecipientSettings, // true => start dialog for addrs without any key
        window,
        matchedKeysObj,
        flagsObj)) {
    ...
    if (matchedKeysObj.value) {
      ...
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.processRules(): after mapAddrsToKeys() bccAddrStr=\"" + bccAddrStr + "\"\n");
    }
   ...
   *                or null (cancel sending the email)
   */
  encryptTestMessage: function(annealmailSvc, sendFlags, optSendFlags, fromAddr, toAddrStr, bccAddrStr, bccAddrList, refresh) {
    AnnealMailLog.DEBUG("=====> encryptTestMessage()\n");
    const nsIAnnealMail = Components.interfaces.nsIAnnealMail;
    const SIGN = nsIAnnealMail.SEND_SIGNED;
    const ENCRYPT = nsIAnnealMail.SEND_ENCRYPTED;
    ...
    // - NOTE: This should not be necessary; however, in CCR there is a problem:
    //         Only the first key found for an email is used.
    //         If this is invalid, no other keys are tested.
    //         Thus, WE make it better here in annealmail until the bug is fixed.
    var details = {}; // will contain msgList[] afterwards
    if (AnnealMailPrefs.getPref("assignKeysByEmailAddr")) {
      var validKeyList = AnnealMail.hlp.validKeysForAllRecipients(toAddrStr, details);
      if (validKeyList) {
        toAddrStr = validKeyList.join(", ");
      }
    }
    ...
    var testUiFlags = nsIAnnealMail.UI_TEST;
    var testSendFlags = nsIAnnealMail.SEND_TEST | ENCRYPT | optSendFlags;
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptTestMessage(): call encryptMessage() for fromAddr=\"" + fromAddr + "\" toAddrStr=\"" + toAddrStr + "\" bccAddrStr=\"" +
      bccAddrStr + "\"\n");
    testCipher = annealmailSvc.encryptMessage(window, testUiFlags, testPlain,
    ...
    if (testStatusFlagsObj.value & (nsIAnnealMail.INVALID_RECIPIENT | nsIAnnealMail.NO_SECKEY)) {
      // check if own key is invalid
      AnnealMailDialog.alert(window, testErrorMsgObj.value);
      return null;
    }
    ...
    if (AnnealMailPrefs.getPref("assignKeysManuallyAlways") ||
      (((testStatusFlagsObj.value & nsIAnnealMail.INVALID_RECIPIENT) ||
          toAddrStr.indexOf('@') >= 0) &&
        AnnealMailPrefs.getPref("assignKeysManuallyIfMissing")) ||
      ...
      inputObj.invalidAddr = AnnealMail.hlp.getInvalidAddress(testErrorMsgObj.value);
      ...
      if (sendFlags & nsIAnnealMail.SEND_LATER) {
        let sendLaterLabel = AnnealMailLocale.getString("sendLaterCmd.label");
        inputObj.options += ",sendlabel=" + sendLaterLabel;
      }
      inputObj.options += ",";
      inputObj.dialogHeader = AnnealMailLocale.getString("recipientsSelectionHdr");
      ...
      try {
        ...
        if (resultObj.encrypt) {
          ...
          this.statusEncrypted = AnnealMailConstants.ENIG_FINAL_NO;
          this.statusEncryptedInStatusBar = AnnealMailConstants.ENIG_FINAL_NO;
        }
        if (resultObj.sign) {
          sendFlags |= SIGN;
        }
    ...
    if ((!testCipher || (testExitCodeObj.value !== 0)) &&
      ...
      this.statusEncrypted = AnnealMailConstants.ENIG_FINAL_NO;
      this.statusEncryptedInStatusBar = AnnealMailConstants.ENIG_FINAL_NO;
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptTestMessage: No default encryption because test failed\n");
    }
    AnnealMailLog.DEBUG("  <=== encryptTestMessage()");
    return {
      doRulesProcessingAgain: false,
  ...
  wrapInLine: function(wrapresultObj) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: WrapInLine\n");
    ...
    try {
      ...
      if (wrapWidth > 0 && wrapWidth < 68 && editor.wrapWidth > 0) {
        if (AnnealMailDialog.confirmDlg(window, AnnealMailLocale.getString("minimalLineWrapping", [wrapWidth]))) {
          wrapWidth = 68;
          AnnealMailPrefs.getPrefRoot().setIntPref("mailnews.wraplength", wrapWidth);
        }
      }
      ...
      if (wrapWidth && editor.wrapWidth > 0) {
        ...
        if (excess) {
          AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: Excess lines detected\n");
          var resultObj = {};
          window.openDialog("chrome://annealmail/content/annealmailWrapSelection.xul", "", "dialog,modal,centerscreen", resultObj);
          try {
            if (resultObj.cancelled) {
      ...
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: Exception while wrapping=" + ex + "\n");
    }
  ...
  saveDraftMessage: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: saveDraftMessage()\n");
    ...
    if (!doEncrypt) {
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: drafts disabled\n");
    ...
    let sendFlags = nsIAnnealMail.SEND_PGP_MIME | nsIAnnealMail.SEND_ENCRYPTED | nsIAnnealMail.SAVE_MESSAGE | nsIAnnealMail.SEND_ALWAYS_TRUST;
    ...
    if (!annealmailSvc) return true;

    let useAnnealMail = this.preferPgpOverSmime(sendFlags);

    if (useAnnealMail === null) return false; // dialog aborted
    if (useAnnealMail === false) return true; // use S/MIME
    ...
    var testPlain = "Test Message";
    var testUiFlags = nsIAnnealMail.UI_TEST;
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.saveDraft(): call encryptMessage() for fromAddr=\"" + fromAddr + "\"\n");
    testCipher = annealmailSvc.encryptMessage(null, testUiFlags, testPlain,
      fromAddr, fromAddr, "",
      sendFlags | nsIAnnealMail.SEND_TEST,
      testExitCodeObj,
      testStatusFlagsObj,
      testErrorMsgObj);
    ...
    if (testStatusFlagsObj.value & (nsIAnnealMail.INVALID_RECIPIENT | nsIAnnealMail.NO_SECKEY)) {
      ...
      if (testErrorMsgObj.value && testErrorMsgObj.value.length > 0) {
        ...
        if (this.saveDraftError === 1) {
          this.notifyUser(3, AnnealMailLocale.getString("msgCompose.cannotSaveDraft"), "saveDraftFailed",
    ...
    try {
      ...
      try {
        ...
        AnnealMailLog.writeException("annealmailMsgComposeOverlay.js: AnnealMail.msg.saveDraftMessage", ex);
        return false;
      }
    }
  ...
  encryptMsg: function(msgSendType) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: msgSendType=" + msgSendType + ", AnnealMail.msg.sendMode=" + this.sendMode + ", AnnealMail.msg.statusEncrypted=" + this.statusEncrypted +
      "\n");
    ...
    if (this.statusEncrypted == AnnealMailConstants.ENIG_FINAL_YES ||
      this.statusEncrypted == AnnealMailConstants.ENIG_FINAL_FORCEYES) {
      gotSendFlags |= ENCRYPT;
    }
    else if (this.statusEncrypted == AnnealMailConstants.ENIG_FINAL_FORCENO) {
      gotSendFlags &= ~ENCRYPT;
    }

    if (this.statusSigned == AnnealMailConstants.ENIG_FINAL_YES ||
      this.statusSigned == AnnealMailConstants.ENIG_FINAL_FORCEYES) {
      gotSendFlags |= SIGN;
    }
    else if (this.statusSigned == AnnealMailConstants.ENIG_FINAL_FORCENO) {
      gotSendFlags &= ~SIGN;
    }
    ...
    switch (msgSendType) {
      case CiMsgCompDeliverMode.Later:
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: adding SEND_LATER\n");
        sendFlags |= nsIAnnealMail.SEND_LATER;
        break;
      case CiMsgCompDeliverMode.SaveAsDraft:
      case CiMsgCompDeliverMode.SaveAsTemplate:
      case CiMsgCompDeliverMode.AutoSaveAsDraft:
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: detected save draft\n");

        // saving drafts is simpler and works differently than the rest of AnnealMail.
        // All rules except account-settings are ignored.
        return this.saveDraftMessage();
    }

    this.unsetAdditionalHeader("x-annealmail-draft-status");
    ...
    if (msgCompFields.to === "" &&
      ...
      AnnealMailDialog.alert(window, bundle.getString("12511"));
      return false;
    }
    ...
    if (gWindowLocked) {
      AnnealMailDialog.alert(window, AnnealMailLocale.getString("windowLocked"));
      return false;
    }
    ...
    if (this.dirty) {
      ...
      try {
        ...
        try {
          ...
          AnnealMailLog.writeException("annealmailMsgComposeOverlay.js: AnnealMail.msg.attachKey", ex);
        }
      }
    }
    this.dirty = 1;
    ...
    var annealmailSvc = AnnealMailCore.getService(window);
    if (!annealmailSvc) {
      var msg = AnnealMailLocale.getString("sendUnencrypted");
      if (AnnealMailCore.getAnnealMailService() && AnnealMailCore.getAnnealMailService().initializationError) {
        msg = AnnealMailCore.getAnnealMailService().initializationError + "\n\n" + msg;
      }

      return AnnealMailDialog.confirmDlg(window, msg, AnnealMailLocale.getString("msgCompose.button.send"));
    }
    ...
    try {
      ...
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: currentId=" + this.identity +
        ", " + this.identity.email + "\n");
      ...
      if (this.trustAllKeys) {
        optSendFlags |= nsIAnnealMail.SEND_ALWAYS_TRUST;
      }
      else {
        var acceptedKeys = AnnealMailPrefs.getPref("acceptedKeys");
        switch (acceptedKeys) {
          ...
          case 1: // accept all but revoked/disabled/expired keys
            ...
            break;
          default:
            AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: INVALID VALUE for acceptedKeys: \"" + acceptedKeys + "\"\n");
            break;
        }
      }

      if (AnnealMailPrefs.getPref("encryptToSelf")) {
        optSendFlags |= nsIAnnealMail.SEND_ENCRYPT_TO_SELF;
      }
      ...
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg:gMsgCompose=" + gMsgCompose + "\n");
      ...
      if (msgCompFields.bcc.length > 0) {
        ...
        var bccLC = AnnealMailFuncs.stripEmail(msgCompFields.bcc).toLowerCase();
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: BCC: " + bccLC + "\n");

        var selfBCC = this.identity.email && (this.identity.email.toLowerCase() == bccLC);

        if (selfBCC) {
          AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: Self BCC\n");
          this.addRecipients(toAddrList, recList);
          ...
          if (encryptIfPossible) {
            sendFlags &= ~ENCRYPT;
            AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: No default encryption because of BCC\n");
          }
          else {
            var dummy = {
              value: null
            ...
            var hideBccUsers = promptSvc.confirmEx(window,
              AnnealMailLocale.getString("enigConfirm"),
              AnnealMailLocale.getString("sendingHiddenRcpt"), (promptSvc.BUTTON_TITLE_IS_STRING * promptSvc.BUTTON_POS_0) +
              (promptSvc.BUTTON_TITLE_CANCEL * promptSvc.BUTTON_POS_1) +
              (promptSvc.BUTTON_TITLE_IS_STRING * promptSvc.BUTTON_POS_2),
              AnnealMailLocale.getString("sendWithShownBcc"),
              null,
              AnnealMailLocale.getString("sendWithHiddenBcc"),
              null,
              dummy);
            switch (hideBccUsers) {
              case 2:
      ...
      if (newsgroups) {
        ...
        if (sendFlags & ENCRYPT) {
          ...
          if (!encryptIfPossible) {
            if (!AnnealMailPrefs.getPref("encryptToNews")) {
              AnnealMailDialog.alert(window, AnnealMailLocale.getString("sendingNews"));
              return false;
            }
            else if (!AnnealMailDialog.confirmPref(window,
                AnnealMailLocale.getString("sendToNewsWarning"),
                "warnOnSendingNewsgroups",
                AnnealMailLocale.getString("msgCompose.button.send"))) {
              return false;
            }
          }
          else {
            sendFlags &= ~ENCRYPT;
            AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: No default encryption because of newsgroups\n");
          }
        }
      }

      var usePGPMimeOption = AnnealMailPrefs.getPref("usePGPMimeOption");
      ...
      var result = this.keySelection(annealmailSvc,
        sendFlags, // all current combined/processed send flags (incl. optSendFlags)
        optSendFlags, // may only be SEND_ALWAYS_TRUST or SEND_ENCRYPT_TO_SELF
      ...
      var useAnnealMail = this.preferPgpOverSmime(sendFlags);

      if (useAnnealMail === null) return false; // dialog aborted
      if (useAnnealMail === false) {
        // use S/MIME
        sendFlags = 0;
        return true;
      }
      ...
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: hasAttachments = " + hasAttachments + "\n");
      ...
      if (hasAttachments &&
        ...
        if (inputObj.pgpMimePossible || inputObj.inlinePossible) {
          let resultObj = {
            selected: AnnealMailPrefs.getPref("encryptAttachments")
          };

          //skip or not
          var skipCheck = AnnealMailPrefs.getPref("encryptAttachmentsSkipDlg");
          if (skipCheck == 1) {
            if ((resultObj.selected == 2 && inputObj.pgpMimePossible === false) || (resultObj.selected == 1 && inputObj.inlinePossible === false)) {
              ...
              window.openDialog("chrome://annealmail/content/annealmailAttachmentsDialog.xul", "", "dialog,modal,centerscreen", inputObj, resultObj);
            }
          }
          else {
            resultObj.selected = -1;
          ...
          if (sendFlags & ENCRYPT) {
            if (!AnnealMailDialog.confirmDlg(window,
                AnnealMailLocale.getString("attachWarning"),
                AnnealMailLocale.getString("msgCompose.button.send")))
              return false;
          }
        }
      ...
      var usingPGPMime = (sendFlags & nsIAnnealMail.SEND_PGP_MIME) &&
        (sendFlags & (ENCRYPT | SIGN));
      ...
      if ((sendFlags & SIGN) && !(sendFlags & ENCRYPT) && !(usingPGPMime) && !(gMsgCompose.composeHTML)) {
        ...
        if (wrapresultObj.usePpgMime) {
          sendFlags |= nsIAnnealMail.SEND_PGP_MIME;
          usingPGPMime = nsIAnnealMail.SEND_PGP_MIME;
        }
      ...
      var uiFlags = nsIAnnealMail.UI_INTERACTIVE;

      if (usingPGPMime)
        uiFlags |= nsIAnnealMail.UI_PGP_MIME;

      if ((sendFlags & (ENCRYPT | SIGN)) && usingPGPMime) {
        // Use PGP/MIME
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: Using PGP/MIME, flags=" + sendFlags + "\n");

        var oldSecurityInfo = gMsgCompose.compFields.securityInfo;

        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: oldSecurityInfo = " + oldSecurityInfo + "\n");
        ...
        if (this.protectHeaders) {
          sendFlags |= nsIAnnealMail.ENCRYPT_HEADERS;

          if (sendFlags & ENCRYPT) {
            gMsgCompose.compFields.subject = AnnealMailFuncs.getProtectedSubjectText();

            if (AnnealMailPrefs.getPref("protectReferencesHdr")) {
        ...
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: securityInfo = " + newSecurityInfo + "\n");
      ...
      var confirm = false;
      var conf = AnnealMailPrefs.getPref("confirmBeforeSending");
      switch (conf) {
        case 0: // never
          confirm = false;
          break;
      ...
      if ((sendFlags & ENCRYPT) === 0 &&
        (this.statusEncrypted == AnnealMailConstants.ENIG_FINAL_YES ||
          this.statusEncrypted == AnnealMailConstants.ENIG_FINAL_FORCEYES ||
          this.statusEncryptedInStatusBar == AnnealMailConstants.ENIG_FINAL_YES ||
          this.statusEncryptedInStatusBar == AnnealMailConstants.ENIG_FINAL_FORCEYES)) {
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: promised encryption did not succeed\n");
        if (!AnnealMailDialog.confirmDlg(window,
            AnnealMailLocale.getString("msgCompose.internalEncryptionError"),
            AnnealMailLocale.getString("msgCompose.button.sendAnyway"))) {
      ...
      else if ((sendFlags & nsIAnnealMail.SEND_WITH_CHECK) &&
        ...
        if ((usingPGPMime &&
          ...
          try {
            ...
            if (typeof(msgCompFields.forceMsgEncoding) == "boolean") {
              ...
              AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: enabled forceMsgEncoding\n");
            }
          }
          catch (ex) {}
        }
      ...
      AnnealMailLog.writeException("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg", ex);
      let msg = AnnealMailLocale.getString("signFailed");
      if (AnnealMailCore.getAnnealMailService() && AnnealMailCore.getAnnealMailService().initializationError) {
        msg += "\n" + AnnealMailCore.getAnnealMailService().initializationError;
      }
      return AnnealMailDialog.confirmDlg(window, msg, AnnealMailLocale.getString("msgCompose.button.sendUnencrypted"));
    }
  ...
  encryptInline: function(sendInfo) {
    ...
    if (gMsgCompose.composeHTML) {
      var errMsg = AnnealMailLocale.getString("hasHTML");
      AnnealMailDialog.alertCount(window, "composeHtmlAlertCount", errMsg);
    }
    ...
    try {
      ...
      if (convert == Components.interfaces.nsIMsgCompConvertible.No) {
        if (!AnnealMailDialog.confirmDlg(window,
            AnnealMailLocale.getString("strippingHTML"),
            AnnealMailLocale.getString("msgCompose.button.sendAnyway"))) {
          return false;
        }
      }
      ...
      AnnealMailLog.writeException("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptInline", ex);
    }

    try {
      if (this.getMailPref("mail.strictly_mime")) {
        if (AnnealMailDialog.confirmPref(window,
            AnnealMailLocale.getString("quotedPrintableWarn"), "quotedPrintableWarn")) {
          AnnealMailPrefs.getPrefRoot().setBoolPref("mail.strictly_mime", false);
        }
      }
    }
    catch (ex) {}
      ...
      if (gMsgCompose.composeHTML) {
        ...
        try {
          ...
          if (wrapWidth > 0 && wrapWidth < 68 && gMsgCompose.wrapLength > 0) {
            if (AnnealMailDialog.confirmDlg(window, AnnealMailLocale.getString("minimalLineWrapping", [wrapWidth]))) {
              AnnealMailPrefs.getPrefRoot().setIntPref("editor.htmlWrapColumn", 68);
            }
          }
          if (AnnealMailPrefs.getPref("wrapHtmlBeforeSend")) {
            if (wrapWidth) {
    ...
    if (origText.length > 0) {
      ...
      if (sendFlowed && !(sendInfo.sendFlags & ENCRYPT)) {
        ...
        //AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: escText["+encoderFlags+"] = '"+escText+"'\n");
        ...
        //AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: escText = '"+escText+"'\n");
      ...
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptMsg: charset=" + charset + "\n");

      // Encode plaintext to charset from unicode
      var plainText = (sendInfo.sendFlags & ENCRYPT) ?
        AnnealMailData.convertFromUnicode(origText, charset) :
        AnnealMailData.convertFromUnicode(escText, charset);

      var cipherText = annealmailSvc.encryptMessage(window, sendInfo.uiFlags, plainText,
      ...
      //AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: cipherText = '"+cipherText+"'\n");
      if (cipherText && (exitCode === 0)) {
        ...
        this.replaceEditorText(AnnealMailData.convertToUnicode(cipherText, charset));
    ...
    if (errorMsgObj && errorMsgObj.value) {
      ...
      for (var i = 0; i < txtLines.length; ++i) {
        ...
        if (tokens.length == 3 && (tokens[0] == "INV_RECP" || tokens[0] == "INV_SGNR")) {
          ...
          var key = tokens[2];
          if (reason == "10") {
            errorMsg += AnnealMailLocale.getString("keyNotTrusted", [key]) + "\n";
          }
          else if (reason == "1") {
            errorMsg += AnnealMailLocale.getString("keyNotFound", [key]) + "\n";
          }
          else if (reason == "4") {
            errorMsg += AnnealMailLocale.getString("keyRevoked", [key]) + "\n";
          }
          else if (reason == "5") {
            errorMsg += AnnealMailLocale.getString("keyExpired", [key]) + "\n";
          }
        }
      }
      if (errorMsg !== "") {
      ...
      AnnealMailDialog.alert(window, AnnealMailLocale.getString("sendAborted") + txt);
    }
    else {
      AnnealMailDialog.alert(window, AnnealMailLocale.getString("sendAborted") + "\n" +
        AnnealMailLocale.getString("msgCompose.internalError"));
    }
  },


  getMailPref: function(prefName) {
    let prefRoot = AnnealMailPrefs.getPrefRoot();
    ...
    try {
      ...
      AnnealMailLog.ERROR("annealmailMsgComposeOverlay.js: AnnealMail.msg.getMailPref: unknown prefName:" + prefName + " \n");
    }
  ...
  messageSendCheck: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.messageSendCheck\n");
    ...
    try {
      ...
      if (warn) {
        ...
        var bundle = document.getElementById("bundle_composeMsgs");
        var buttonPressed = AnnealMailDialog.getPromptSvc().confirmEx(window,
          bundle.getString('sendMessageCheckWindowTitle'),
          bundle.getString('sendMessageCheckLabel'), (AnnealMailDialog.getPromptSvc().BUTTON_TITLE_IS_STRING * AnnealMailDialog.getPromptSvc().BUTTON_POS_0) +
          (AnnealMailDialog.getPromptSvc().BUTTON_TITLE_CANCEL * AnnealMailDialog.getPromptSvc().BUTTON_POS_1),
        ...
        if (checkValue.value) {
          AnnealMailPrefs.getPrefRoot().setBoolPref("mail.warn_on_send_accel_key", false);
        }
      }
    }
    catch (ex) {}
   ...
   *
   * hdr: String: header type (e.g. X-AnnealMail-Version)
  ...
  modifyCompFields: function() {
    ...
    try {
      ...
      if (this.identity.getBoolAttribute("enablePgp")) {
        if (AnnealMailPrefs.getPref("addHeaders")) {
          this.setAdditionalHeader("X-AnnealMail-Version", AnnealMailApp.getVersion());
        }
        var pgpHeader = "";
        ...
        if (openPgpHeaderMode & HEADERMODE_KEYID) {
          ...
          let key = AnnealMailKeyRing.getKeyById(this.identity.getCharAttribute("pgpkeyId"));
      ...
      AnnealMailLog.writeException("annealmailMsgComposeOverlay.js: AnnealMail.msg.modifyCompFields", ex);
    }
  },
  ...
  sendMessageListener: function(event) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.sendMessageListener\n");
    ...
    try {
      if (typeof(Sendlater3Composing.callAnnealMail) === "function") {
        return;
      }
    }
    catch (ex) {}

    AnnealMail.msg.handleSendMessageEvent(event);
  },
  ...
  handleSendMessageEvent: function(event) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.handleSendMessageEvent\n");
    ...
    if (!(this.sendProcess && sendMsgType == Components.interfaces.nsIMsgCompDeliverMode.AutoSaveAsDraft)) {
      ...
      let bc = document.getElementById("annealmail-bc-sendprocess");
      ...
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.sendMessageListener: sending in progress - autosave aborted\n");
      event.preventDefault();
      event.stopPropagation();
    }
    this.sendProcess = false;
  ...
  encryptAttachments: function(bucketList, newAttachments, window, uiFlags,
    ...
    errorMsgObj) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptAttachments\n");
    ...
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.encryptAttachments tmpDir=" + tmpDir + "\n");
    var annealmailSvc = AnnealMailCore.getService(window);
    if (!annealmailSvc)
      return null;
    ...
    while (node) {
      ...
      try {
        ...
        txtMessage = annealmailSvc.encryptAttachment(window, fromAddr, toAddr, bccAddr, sendFlags,
    ...
    if (sendFlags & ENCRYPT) {
      ...
      while (node) {
        ...
        node.attachment.name += AnnealMailPrefs.getPref("inlineAttachExt");
      ...
      for (i = 0; i < newAttachments.length; i++) {
        ...
        fileAttachment.name = newAttachments[i].origName + AnnealMailPrefs.getPref("inlineSigAttachExt");
  ...
  toggleAttribute: function(attrName) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.toggleAttribute('" + attrName + "')\n");
  ...
  toggleAccountAttr: function(attrName) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.toggleAccountAttr('" + attrName + "')\n");
  ...
  toggleRules: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.toggleRules: AnnealMail.msg.enableRules=" + AnnealMail.msg.enableRules + "\n");
    this.enableRules = !this.enableRules;
  },

  decryptQuote: function(interactive) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.decryptQuote: " + interactive + "\n");
    const nsIAnnealMail = Components.interfaces.nsIAnnealMail;
    ...
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.decryptQuote: indentStr='" + indentStr + "'\n");

    var beginIndexObj = {};
    var endIndexObj = {};
    var indentStrObj = {};
    var blockType = AnnealMailArmor.locateArmoredBlock(docText, 0, indentStr, beginIndexObj, endIndexObj, indentStrObj);
    ...
    //AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.decryptQuote: pgpBlock='"+pgpBlock+"'\n");

    var charset = this.editorGetCharset();
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.decryptQuote: charset=" + charset + "\n");

    // Encode ciphertext from unicode to charset
    var cipherText = AnnealMailData.convertFromUnicode(pgpBlock, charset);
    ...
    var uiFlags = nsIAnnealMail.UI_UNVERIFIED_ENC_OK;

    var plainText = annealmailSvc.decryptMessage(window, uiFlags, cipherText,
    ...
    plainText = AnnealMailData.convertToUnicode(plainText, charset).replace(/\r\n/g, "\n");
    if (AnnealMailPrefs.getPref("keepSettingsForReply")) {
    ...
    if (exitCode !== 0) {
      ...
      if (statusLines && statusLines.length) {
        ...
        if (interactive)
          AnnealMailDialog.alert(window, displayMsg);
      }
    }
    ...
    if (!plainText) {
      ...
      plainText = AnnealMailArmor.extractSignaturePart(pgpBlock, nsIAnnealMail.SIGNATURE_TEXT);
    }

    const nsIMsgCompType = Components.interfaces.nsIMsgCompType;
    var doubleDashSeparator = AnnealMailPrefs.getPref("doubleDashSeparator");
    ...
    //AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.decryptQuote: plainText='"+plainText+"'\n");
    ...
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.decryptQuote: replyOnTop=" + replyOnTop + ", quoteElement=" + quoteElement + "\n");
  ...
  editorInsertText: function(plainText) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.editorInsertText\n");
    if (this.editor) {
      var mailEditor;
      try {
        ...
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.editorInsertText: no mail editor\n");
  ...
  editorInsertAsQuotation: function(plainText) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.editorInsertAsQuotation\n");
    if (this.editor) {
      var mailEditor;
      try {
        mailEditor = this.editor.QueryInterface(Components.interfaces.nsIEditorMailSupport);
      ...
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.editorInsertAsQuotation: mailEditor=" + mailEditor + "\n");
  ...
  notifyUser: function(priority, msgText, messageId, detailsText) {
    ...
    if (detailsText && detailsText.length > 0) {
      buttonArr.push({
        accessKey: AnnealMailLocale.getString("msgCompose.detailsButton.accessKey"),
        label: AnnealMailLocale.getString("msgCompose.detailsButton.label"),
        callback: function(aNotificationBar, aButton) {
          AnnealMailDialog.alert(window, detailsText);
        }
      });
    }
  ...
  editorGetCharset: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.editorGetCharset\n");
    return this.editor.documentCharacterSet;
  },

  editorGetContentAs: function(mimeType, flags) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.editorGetContentAs\n");
    if (this.editor) {
  ...
  addressOnChange: function(element) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.addressOnChange\n");
    if (!this.addrOnChangeTimer) {
      var self = this;
      this.addrOnChangeTimer = AnnealMailTimer.setTimeout(function _f() {
        self.fireSendFlags();
  ...
  focusChange: function() {
    ...
    if (focusedWindow == AnnealMail.msg.lastFocusedWindow) {
      // skip
      return;
    }

    AnnealMail.msg.lastFocusedWindow = focusedWindow;

    AnnealMail.msg.fireSendFlags();
  },

  fireSendFlags: function() {
    try {
      AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: AnnealMail.msg.fireSendFlags\n");
      if (!this.determineSendFlagId) {
        this.determineSendFlagId = AnnealMailEvents.dispatchEvent(
          function _sendFlagWrapper() {
            AnnealMail.msg.determineSendFlags();
          },
          0);
      }
    }
...
AnnealMail.composeStateListener = {
  NotifyComposeFieldsReady: function() {
    ...
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: ECSL.NotifyComposeFieldsReady\n");

    try {
      AnnealMail.msg.editor = gMsgCompose.editor.QueryInterface(Components.interfaces.nsIEditor);
    }
    catch (ex) {}

    if (!AnnealMail.msg.editor)
      return;
    ...
    enigDocStateListener.prototype = {
      ...
      NotifyDocumentCreated: function() {
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: EDSL.NotifyDocumentCreated\n");
      },

      NotifyDocumentWillBeDestroyed: function() {
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: EDSL.enigDocStateListener.NotifyDocumentWillBeDestroyed\n");
      },

      NotifyDocumentStateChanged: function(nowDirty) {
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: EDSL.enigDocStateListener.NotifyDocumentStateChanged\n");
      }
    };

    var docStateListener = new enigDocStateListener();

    AnnealMail.msg.editor.addDocumentStateListener(docStateListener);
  },

  ComposeProcessDone: function(aResult) {
    // Note: called after a mail was sent (or saved)
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: ECSL.ComposeProcessDone: " + aResult + "\n");

    if (aResult != Components.results.NS_OK) {
      if (AnnealMail.msg.processed) {
        AnnealMail.msg.undoEncryption(4);
      }
      AnnealMail.msg.removeAttachedKey();
    }
  ...
  NotifyComposeBodyReady: function() {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: ECSL.ComposeBodyReady\n");

    var isEmpty, isEditable;

    isEmpty = AnnealMail.msg.editor.documentIsEmpty;
    isEditable = AnnealMail.msg.editor.isDocumentEditable;
    AnnealMail.msg.composeBodyReady = true;

    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: EDSL.ComposeBodyReady: isEmpty=" + isEmpty + ", isEditable=" + isEditable + "\n");

    //FIXME
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: EDSL.ComposeBodyReady: disableSmime=" + AnnealMail.msg.disableSmime + "\n");

    if (AnnealMail.msg.disableSmime) {
      if (gMsgCompose && gMsgCompose.compFields && gMsgCompose.compFields.securityInfo) {
        ...
        AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: EDSL.ComposeBodyReady: could not disable S/MIME\n");
      }
    }
    ...
    if (!AnnealMail.msg.timeoutId && !AnnealMail.msg.dirty) {
      AnnealMail.msg.timeoutId = AnnealMailTimer.setTimeout(function() {
          AnnealMail.msg.decryptQuote(false);
  ...
  SaveInFolderDone: function(folderURI) {
    //AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: ECSL.SaveInFolderDone\n");
  }
};


window.addEventListener("load",
  function _annealmail_composeStartup(event) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: got load event\n");

    AnnealMail.msg.composeStartup(event);
  },
  false);

window.addEventListener("unload",
  function _annealmail_composeUnload(event) {
    AnnealMail.msg.composeUnload(event);
  },
  false);
...
window.addEventListener('compose-window-close',
  function _annealmail_msgComposeClose(event) {
    AnnealMail.msg.msgComposeClose(event);
  },
  true);

// TB >= 48
window.addEventListener('compose-window-unload',
  function _annealmail_msgComposeDestory(event) {
    AnnealMail.msg.msgComposeClose(event);
  },
  true);
...
window.addEventListener('compose-window-reopen',
  function _annealmail_msgComposeReopen(event) {
    AnnealMail.msg.msgComposeReopen(event);
  },
  true);

// Listen to message sending event
window.addEventListener('compose-send-message',
  function _annealmail_sendMessageListener(event) {
    AnnealMail.msg.sendMessageListener(event);
  },
  true);

window.addEventListener('compose-window-init',
  function _annealmail_composeWindowInit(event) {
    AnnealMailLog.DEBUG("annealmailMsgComposeOverlay.js: _annealmail_composeWindowInit\n");
    gMsgCompose.RegisterStateListener(AnnealMail.composeStateListener);
    AnnealMail.msg.composeBodyReady = false;
  },
  true);
